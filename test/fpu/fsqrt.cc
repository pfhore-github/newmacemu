...
BOOST_FIXTURE_TEST_SUITE(FSQRT, Prepare)
...
BOOST_FIXTURE_TEST_SUITE(FSSQRT, Prepare, *boost::unit_test::tolerance(1e-5))
BOOST_AUTO_TEST_CASE(qnan) {
    mpfr_set_nan(cpu.FP[3]);
    cpu.FP_nan[3] = 0xffff000000000000LLU;
    TEST::SET_W(0, 0171000);
    TEST::SET_W(2, 0B1000001 | 3 << 10 | 2 << 7);
...
BOOST_FIXTURE_TEST_SUITE(FDSQRT, Prepare)
BOOST_AUTO_TEST_CASE(qnan) {
    mpfr_set_nan(cpu.FP[3]);
    cpu.FP_nan[3] = 0xffff000000000000LLU;
    TEST::SET_W(0, 0171000);
    TEST::SET_W(2, 0B1000101 | 3 << 10 | 2 << 7);
    int i = decode_and_run();
    BOOST_TEST(mpfr_nan_p(cpu.FP[2]));
    BOOST_TEST(cpu.FP_nan[2] == 0xffff000000000000LLU);
    BOOST_TEST(i == 2);
