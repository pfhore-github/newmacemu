...
struct F_MOVE {
    F_MOVE() {
        ...
        TEST::SET_L(102, 0x1000);
        TEST::SET_L(106, 0x1200);
        ...
        TEST::SET_L(118, 0x1000);
        TEST::SET_L(122, 0x1200);
        ...
        TEST::SET_L(142, 0x1000);
        TEST::SET_L(146, 0x1200);
...
BOOST_FIXTURE_TEST_SUITE(MOVE, Prepare, *boost::unit_test::fixture<F_MOVE>())
...
BOOST_AUTO_TEST_CASE(NG) {
    cpu.S = false;
    run_test(60, EXCEPTION_NUMBER::PRIV_ERR);
}
...
BOOST_AUTO_TEST_CASE(NG) {
    cpu.S = false;
    run_test(92, EXCEPTION_NUMBER::PRIV_ERR);
}
...
BOOST_AUTO_TEST_CASE(Traced) {
    cpu.S = true;
    cpu.T = 1;
    run_test(92, EXCEPTION_NUMBER::TRACE);
}
BOOST_AUTO_TEST_SUITE_END()
...
BOOST_AUTO_TEST_CASE(mem2mem) {
    ...
    RAM[0x1200] = 0xff;
    run_test(100);
    BOOST_TEST(RAM[0x1200] == 0x02);
}
BOOST_AUTO_TEST_SUITE_END()
BOOST_AUTO_TEST_SUITE(Word)
BOOST_AUTO_TEST_CASE(mem) {
    TEST::SET_W(0x1000, 0x0234);
    cpu.D[3] = 0x12345678;
    cpu.A[2] = 0x1000;
    run_test(112);
    BOOST_TEST(TEST::GET_W(0x1000) == 0x5678);
}

BOOST_AUTO_TEST_CASE(mem2mem) {
    TEST::SET_W(0x1000, 0x1234);
    TEST::SET_W(0x1200, 0x5678);
    run_test(116);
...
BOOST_AUTO_TEST_CASE(NG) {
    cpu.S = false;
    run_test(132, EXCEPTION_NUMBER::PRIV_ERR);
}
...
BOOST_AUTO_TEST_CASE(Traced) {
    ...
    cpu.T = 1;
    cpu.D[3] = 1 << 14;
    run_test(132, EXCEPTION_NUMBER::TRACE);
}
BOOST_AUTO_TEST_SUITE_END()
BOOST_AUTO_TEST_SUITE_END()
BOOST_AUTO_TEST_SUITE(Long)
BOOST_AUTO_TEST_CASE(mem) {
    TEST::SET_L(0x1000, 0x02347654);
    cpu.D[3] = 0x12345678;
    cpu.A[2] = 0x1000;
    run_test(136);
...
BOOST_AUTO_TEST_CASE(mem2mem) {
    ...
    TEST::SET_L(0x1200, 0x56789ABC);
    run_test(140);
    BOOST_TEST(TEST::GET_L(0x1200) == 0x12345678);
}
BOOST_AUTO_TEST_SUITE(USP)
BOOST_AUTO_TEST_CASE(NG) {
    cpu.S = false;
    run_test(152, EXCEPTION_NUMBER::PRIV_ERR);
}
...
BOOST_AUTO_TEST_CASE(Traced) {
    cpu.S = true;
    cpu.T = 1;
    run_test(152, EXCEPTION_NUMBER::TRACE);
}
BOOST_AUTO_TEST_SUITE_END()
BOOST_AUTO_TEST_SUITE_END()
BOOST_AUTO_TEST_SUITE_END()
